document.addEventListener('DOMContentLoaded', () => {
    ...
    resetBtn.addEventListener('click', () => { // 添加重置按钮的点击事件
        if (confirm('确定要重置游戏吗？')) { // 弹出确认对话框
            ...
            renderLevel(); // 重新渲染关卡
            localStorage.removeItem('currentLevel'); // 清除历史数据
            localStorage.removeItem('totalScore'); // 清除总分数据
            localStorage.removeItem('levelTimes'); // 清除关卡时间数据
    ...
    function loadLevelData() {
        ...
        iconBase64 = localStorage.getItem('iconBase64') || 'p.png'; // 从本地存储中获取图片
    ...
    iconInput.addEventListener('change', (e) => {
        ...
        createMovableIcon(); // 更新图标
        localStorage.setItem('iconBase64', iconBase64); // 保存到本地存储
    });
